...
static void blinky_work_work_handler(struct k_work *work) {
    ...
    k_work_reschedule(&blinky_work, K_MSEC(CONFIG_ZMK_IDICATOR_BLINK_INTERVAL));
}
...
static void polling_work_work_handler(struct k_work *work) {
    if (zmk_keymap_highest_layer_active() == 0) {
        ...
        if (!zmk_indicator_led_is_on()) zmk_indicator_led_on();
        cycle_onoff(true);
        blinky_onoff(false);
    } else if (zmk_keymap_highest_layer_active() == 4) {
        if (!zmk_indicator_led_is_on()) zmk_indicator_led_on();
        cycle_onoff(false);
        blinky_onoff(true);
    }
